...
## Continue with an External Agent

After MCP is connected, ask the agent to inspect the same schema and extend the analysis with read-only SQL. Dory preserves the generated SQL, result snapshots, and findings in an Agent Run so you can open the work, verify it, edit the query, and rerun it yourself.

## Next Step

Continue with [SQL Console](/en/docs/core-features/sql-console), [AI Chat](/en/docs/core-features/ai-chat), or [Agent Workspaces](/en/docs/agents).
