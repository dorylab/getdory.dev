...
Dory can connect to multiple database types. After a connection is added, people can use Explorer, SQL Console, and AI Chat, while connected agents can inspect the same schema and run read-only SQL through MCP.
...
- For SQL Server, confirm database, schema permissions, SSL requirements, and T-SQL compatibility.
- For Oracle, confirm service name, schema permissions, and Oracle SQL syntax requirements.
