...
支持的 provider 包括 `openai`、`openai-compatible`、`anthropic`、`google`、`qwen`、`xai` 和 `meta`。不同 provider 对 `DORY_AI_URL` 的要求不同，详情见 [AI Provider 配置](/zh/docs/reference/ai-providers)。
...
- 阅读 [环境变量配置](/zh/docs/deploy/environment-variables) 完整梳理配置。
- 阅读 [自托管部署](/zh/docs/deploy/self-hosting) 完成团队上线准备。
- 阅读 [云数据库](/zh/docs/database-support/cloud-databases) 规划数据库网络访问。
