...
支持的 provider 包括 `openai`、`openai-compatible`、`anthropic`、`google`、`qwen`、`xai` 和 `meta`。不同 provider 对 `DORY_AI_URL` 的要求不同，详情见 [AI Providers](../reference/ai-providers)。
...
Dory 容器会在运行时写入应用数据。生产环境应挂载持久化目录，避免容器重建后数据丢失。
...
- 阅读 [Environment Variables](./environment-variables) 完整梳理配置。
- 阅读 [Self-Hosting](./self-hosting) 完成团队上线准备。
- 阅读 [Cloud Databases](../database-support/cloud-databases) 规划数据库网络访问。
