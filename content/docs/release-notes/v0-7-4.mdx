...
- **Guest session recovery**: Guest sessions are now reliably restored *before* sign-in begins (both web and desktop), improving continuity after logout or app restart.
...
- **desktop OAuth flow**: Corrected session handling in OAuth callback (`finalize`) and token consumption (`consume`) routes to prevent auth failures.
