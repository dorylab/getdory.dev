...
打开 [Explorer](../core-features/explorer)，先确认数据库、Schema、表和视图是否能正常加载。
...
在 [SQL Console](../core-features/sql-console) 中先运行小范围查询：
...
当你已经确认目标表后，可以向 [AI Chat](../core-features/ai-chat) 提问：
...
- 用 [Generate SQL with AI](./generate-sql-with-ai) 生成第一版业务查询。
- 在 [SQL Console](../core-features/sql-console) 中运行并验证结果。
- 输出稳定后使用 [Build Charts from SQL](./build-charts-from-sql) 做可视化。
